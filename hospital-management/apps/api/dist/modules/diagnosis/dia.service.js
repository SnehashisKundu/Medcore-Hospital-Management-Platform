...
async function createDiagnosis(input) {
    ...
    if (input.isPrimary === true) {
        await prisma_1.prisma.encounterDiagnosis.updateMany({
            where: {
    ...
    // 5. Create diagnosis
    return prisma_1.prisma.encounterDiagnosis.create({
        data: {
            encounterId: input.encounterId,
...
async function getDiagnoses() {
    return prisma_1.prisma.encounterDiagnosis.findMany({
        include: {
            encounter: true,
            diagnosedBy: {
                select: {
...
async function getDiagnosisById(id) {
    const diagnosis = await prisma_1.prisma.encounterDiagnosis.findUnique({
        where: {
            id,
        },
        include: {
...
async function updateDiagnosis(id, input) {
    const diagnosis = await prisma_1.prisma.encounterDiagnosis.findUnique({
        where: {
            id,
        },
    ...
    return prisma_1.prisma.encounterDiagnosis.update({
        where: {
            id,
        },
        data: {
