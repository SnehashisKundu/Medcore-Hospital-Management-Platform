...
exports.getImagingReportById = getImagingReportById;
...
async function createImagingReport(input) {
    ...
    const existing = await prisma_1.prisma.imagingReport.findUnique({
        where: {
...
async function getImagingReports() {
    return prisma_1.prisma.imagingReport.findMany({
        include: {
            diagnosticOrderItem: {
                include: {
                    diagnosticTest: true,
...
async function getImagingReportById(id) {
    const report = await prisma_1.prisma.imagingReport.findUnique({
        where: {
            id,
        },
        include: {
...
async function updateImagingReport(id, input) {
    const report = await prisma_1.prisma.imagingReport.findUnique({
        where: {
            id,
        },
    ...
    const hasUpdate = input.findings !== undefined ||
    ...
    return prisma_1.prisma.imagingReport.update({
        where: {
            id,
        },
        data,
