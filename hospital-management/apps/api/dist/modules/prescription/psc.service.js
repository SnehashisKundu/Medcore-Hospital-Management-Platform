...
exports.getPrescriptionPdfData = getPrescriptionPdfData;
...
function normalizeEnumLikeValue(value) {
    return value
        .trim()
        .toUpperCase()
        .replace(/[\s-]+/g, "_");
}
function normalizeMedicineRoute(route) {
    if (route === undefined ||
        route === null) {
        return undefined;
    }
    ...
    const normalized = normalizeEnumLikeValue(route);
    const mappedRoute = MEDICINE_ROUTE_ALIASES[normalized];
    if (!mappedRoute) {
        throw new Error("INVALID_MEDICINE_ROUTE");
    }
    return mappedRoute;
}
function normalizeMedicineTiming(timing) {
    if (timing === undefined ||
        timing === null) {
        return undefined;
    }
    ...
    const normalized = normalizeEnumLikeValue(timing);
    const mappedTiming = MEDICINE_TIMING_ALIASES[normalized];
    if (!mappedTiming) {
        throw new Error("INVALID_MEDICINE_TIMING");
    }
    return mappedTiming;
...
function validatePrescriptionItem(item) {
    ...
    if (typeof prescriptionItem.medicineId !==
        "string" ||
    ...
    if (prescriptionItem.quantity !== undefined &&
        (typeof prescriptionItem.quantity !==
            "number" ||
            !Number.isFinite(prescriptionItem.quantity) ||
            !Number.isInteger(prescriptionItem.quantity) ||
            prescriptionItem.quantity <= 0)) {
        throw new Error("INVALID_QUANTITY");
    }
    if (prescriptionItem.remarks !== undefined &&
        prescriptionItem.remarks !== null &&
        typeof prescriptionItem.remarks !==
            "string") {
        throw new Error("INVALID_PRESCRIPTION_ITEM");
    }
    normalizeMedicineRoute(prescriptionItem.route);
    normalizeMedicineTiming(prescriptionItem.timing);
}
function validatePrescriptionId(id) {
    if (typeof id !== "string" ||
        !id.trim()) {
        throw new Error("INVALID_PRESCRIPTION_ID");
    }
}
...
async function createPrescription(input) {
    if (typeof input.encounterId !== "string" ||
        !input.encounterId.trim()) {
        throw new Error("ENCOUNTER_NOT_FOUND");
    }
    if (typeof input.prescribedById !==
        "string" ||
    ...
    const prescribedById = input.prescribedById.trim();
    ...
    const prescriber = await prisma_1.prisma.user.findUnique({
        where: {
            id: prescribedById,
        },
        select: {
    ...
    const inactiveMedicine = medicines.find((medicine) => !medicine.isActive);
    if (inactiveMedicine) {
        throw new Error("MEDICINE_INACTIVE");
    }
            ...
            data: {
                ...
                items: {
                    create: input.items.map((item) => ({
                        ...
                        route: normalizeMedicineRoute(item.route),
                        timing: normalizeMedicineTiming(item.timing),
...
async function getPrescriptionPdfData(id) {
    validatePrescriptionId(id);
    const prescription = await prisma_1.prisma.prescription.findUnique({
        where: {
            id: id.trim(),
...
async function updatePrescription(id, input) {
    validatePrescriptionId(id);
    if (!input ||
        typeof input !== "object" ||
        Array.isArray(input)) {
    ...
    if (prescription.encounter.status ===
        "CANCELLED") {
        throw new Error("ENCOUNTER_CANCELLED");
    }
    ...
    if (prescription.status !== "ACTIVE") {
        throw new Error("PRESCRIPTION_NOT_ACTIVE");
    }
