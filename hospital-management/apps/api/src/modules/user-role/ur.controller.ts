    ...
    if (error instanceof Error) {
      ...
      > = {
        ...
        HOSPITAL_ID_REQUIRED: {
          status: 400,
          message:
            "hospitalId is required for this role",
        },
        SUPER_ADMIN_CANNOT_HAVE_HOSPITAL: {
          status: 400,
          message:
            "SUPER_ADMIN cannot be assigned to a hospital",
        },
...
export async function getAllUserRolesController(
  req: AuthRequest,
  res: Response
) {
  try {
...
export async function getUserRolesByUserIdController(
  ...
  try {
    ...
    const data = await getUserRolesByUserId(
      normalizedUserId
    );
