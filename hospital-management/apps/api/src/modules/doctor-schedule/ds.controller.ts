  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Doctor hospital ID, department ID, day of week, start time, end time and slot duration are required",
      });
    }
    ...
    await createAuditLog({
      userId: req.user?.id,
      hospitalId:
        schedule.doctorHospital?.hospitalId,
      action: "CREATE",
      entityType: "DOCTOR_SCHEDULE",
      entityId: schedule.id,
      metadata: {
        doctorHospitalId:
          schedule.doctorHospitalId,
...
export async function getDoctorSchedulesController(
  _req: Request,
  res: Response
) {
  try {
...
export async function getDoctorScheduleByIdController(
  req: Request,
  res: Response
) {
  try {
...
export async function updateDoctorScheduleController(
  req: AuthRequest,
  res: Response
) {
  try {
...
export async function deleteDoctorScheduleController(
  req: AuthRequest,
  res: Response
) {
  try {
